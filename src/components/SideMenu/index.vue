  ...
  <a-layout style="height:100%">
    ...
    <a-layout style="padding: 0 10px 10px">
      ...
      <a-breadcrumb style="margin: 10px 0">
        <a-breadcrumb-item v-for="(v, i) in breadcrumb" :key="i">{{v}}</a-breadcrumb-item>
      </a-breadcrumb>
      <!-- 页面内容区 -->
...
export default {
  ...
  computed: {
    //获取二级菜单
    menus() {
      return this.$store.getters['common/user/getCurrentMenu']['children']
    },
  },
  watch: {
    collapsed(val) {
    ...
    $route: {
      handler(route) {
        this.openKeys = [route.matched[1].path]
